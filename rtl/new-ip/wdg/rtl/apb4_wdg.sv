...
`ifndef __VERILOG__
  ...
  logic wdg_rtc_clk_i = wdg.rtc_clk_i;
  logic wdg_rst_o;
  assign wdg.rst_o = wdg_rst_o;
`endif
  ...
  assign wdg_rst_o       = s_bit_ovif;

  assign s_wdg_ctrl_en   = s_apb4_wr_hdshk && s_apb4_addr == `WDG_CTRL && s_key_match;
  assign s_wdg_ctrl_d    = apb4_pwdata[`WDG_CTRL_WIDTH-1:0];
  ...
  assign s_wdg_pscr_d  = apb4_pwdata[`WDG_PSCR_WIDTH-1:0];
  ...
  assign s_wdg_cmp_d  = apb4_pwdata[`WDG_CMP_WIDTH-1:0];
  ...
  always_comb begin
    ...
    if (s_apb4_wr_hdshk) begin
      if (s_apb4_addr == `WDG_KEY) begin
        s_wdg_key_d = apb4_pwdata[`WDG_KEY_WIDTH-1:0];
      end else begin
        s_wdg_key_d = '0;
      end
    end
  ...
  assign s_wdg_feed_d  = apb4_pwdata[`WDG_FEED_WIDTH-1:0];
  ...
  always_comb begin
    ...
    if (s_apb4_rd_hdshk) begin
      unique case (s_apb4_addr)
        `WDG_CTRL: apb4_prdata[`WDG_CTRL_WIDTH-1:0] = s_wdg_ctrl_q;
        `WDG_PSCR: apb4_prdata[`WDG_PSCR_WIDTH-1:0] = s_wdg_pscr_q;
        `WDG_CMP:  apb4_prdata[`WDG_CMP_WIDTH-1:0] = s_wdg_cmp_q;
        `WDG_STAT: apb4_prdata[`WDG_STAT_WIDTH-1:0] = s_wdg_stat_q;
        `WDG_KEY:  apb4_prdata[`WDG_KEY_WIDTH-1:0] = s_wdg_key_q;
        `WDG_FEED: apb4_prdata[`WDG_FEED_WIDTH-1:0] = s_wdg_feed_q;
